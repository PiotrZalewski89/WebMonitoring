...
{
    public class Stf2 : BaseSearchClass
    {      
        private Stf2DbContext context { get; set; }
        private CodeStf Codes { get; set; }

        public Stf2()
        {
            context = new Stf2DbContext();
        ...
        private CodeStf FindCode(string code)
        {
            ...
            if (result.Length == 0)
            {
                result = context.TblSubAssemblies
                 .Where(x => x.VShellSerialNum.Contains(code) || x.VShellSerialNum2.Contains(code) || x.VBrick2SerialNum.Contains(code) || x.VBrick1SerialNum.Contains(code) || x.VMat1SerialNum.Contains(code) || x.VMat2SerialNum.Contains(code))
                 .Select(x => new CodeStf
                 {
                     Shell1 = x.VShellSerialNum,
                     Shell2 = x.VShellSerialNum2,
                ...
                foreach (var code in codeList)
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(Codes.Brick1) && Codes.Brick1 != Brak)
                    {
                        var result = context.TblBricks
                                .Where(x => x.VBrickSerialNum == Codes.Brick1);
                    ...
                    {
                        var result = context.TblBricks
                                .Where(x => x.VBrickSerialNum == Codes.Brick2);
                    ...
                    if (!string.IsNullOrWhiteSpace(Codes.Mat1) && Codes.Mat1 != Brak)
                    {
                        var result = context.TblMats
                                .Where(x => x.VMatSerialNum == Codes.Mat1);
                    ...
                    {
                        var result = context.TblMats
                                .Where(x => x.VMatSerialNum == Codes.Mat2);
                    ...
                    if (!string.IsNullOrWhiteSpace(Codes.Shell1) && Codes.Shell1 != Brak)
                    {
                        ...
                        if (resultArray.Length > 0)
                        {
                            ...
                            {
                                Table.Add(dataTable.WriteTextToFile(result));
                                FileName.Add("TblShells");
                            }
                            else
                            {
                                NameTable.Add("Shell1");
                                HtmlTable.Add(dataTable.WriteToTable(resultArray));
                            }
                        }
                    }
        ...
        public void GetDataFromSql(DateTime? dateTime, bool download = false)
        {
            ...
            try
            {
                ...
                var result6 = context.TblSubAssemblies
                         .Where(x => x.DtAssembly >= from && x.DtAssembly < to);

                var resultArray6 = result6.ToArray();
                if (resultArray6.Length > 0)
                {
                    if (download)
                    {
                        Table.Add(dataTable.WriteTextToFile(result6));
                        FileName.Add("TblSubAssemblies");
                    }
                    else
                        HtmlTable.Add(dataTable.WriteToTable(resultArray6));
                }
            }
            catch { }
