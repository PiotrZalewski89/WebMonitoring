...
{
    public class CheckBoxLineWs2 : BaseSearchClass
    {
        private StorageStationDbContext context { get; set; }
        private Codes Codes { get; set; }

        public bool All { get; set; }
        public bool CeleSpawalnicza1_2 { get; set; }
        ...
        public IList<BasicColumn> DaneCelaSprawalnicza1_2 { get; set; }
        ...
        {
            var result = context.PreleaktesterHr16L3s
            ...
            if (result.Length == 0)
            {
                result = context.PreleaktesterHr16L3s
        ...
        private IList<string> FindCodeByPzzw(string find)
        {
            ...
            var codeFromPzzw = context.PetlaKontrolnaHr16L3s
        ...
        private IList<BasicColumn> GetDataFromWeldingCell1_2(string code)
        {
            var result = context.Cela1And2Hr16L3s
        ...
        {
            var result = context.Cela3Hr16L3s
                          .Where(x => x.NrShella == code)
                          .Select(x => new BasicColumn
                          {
                              Nr_Shell = x.NrShella,
        ...
        {
            var result = context.FinalLeaktesterHr16L3s
                    .Where(x => x.NrGrawerka == code)
                    .Select(x => new ColumnLT
                    {
                        Nr_Shell = x.NrShella,
        ...
        {
            var result = context.FinalGaugeHr16L3s
                    .Where(x => x.NrGrawerka == code)
                    .Select(x => new BasicColumn
                    {
                        Nr_Shell = x.NrShella,
        ...
        {
            var result = context.OdkurzaczHr16L3s
                    .Where(x => x.NrGrawerka == code)
        ...
        {
            var result = context.PetlaKontrolnaHr16L3s
                     .Where(x => x.NrGrawerka == code)
                     .OrderByDescending(x => x.Id)
        ...
        private IList<BasicColumn> GetDataFromWeldingCell1_2_ByDate(DateTime from, DateTime to)
        {
        ...
        {
            DaneCelaSprawalnicza1_2 = new List<BasicColumn>();
            ...
            if (SelectCode)
            {
                foreach (var code in FindData)
                {
                    ...
                    if (CeleSpawalnicza1_2 && Codes.CodeBasic != Brak)
                    {
                        var result = GetDataFromWeldingCell1_2(Codes.CodeBasic);

                        foreach (var r in result)
                        {
                            DaneCelaSprawalnicza1_2.Add(r);
                        }
                    }
                    ...
                    if (CeleSpawalnicza3 && Codes.CodeCatalyst != Brak)
                    {
                        var result = GetDataFromWeldingCell3(Codes.CodeCatalyst);
            ...
            else if (SelectDate)
            {
                ...
                if (CeleSpawalnicza1_2)
                {
                    var result = GetDataFromWeldingCell1_2_ByDate(from, to);
        ...
        private void WriteData()
        {
            ...
            if (DaneCelaSprawalnicza1_2?.Count > 0)
            {
                Table.Add(dataTable.WriteTextToFile(DaneCelaSprawalnicza1_2.ToArray()));
                FileName.Add("CelaSpawalnicza 1 i 2");
            }
